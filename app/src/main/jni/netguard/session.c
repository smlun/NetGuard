...
void *handle_events(void *a) {
    syslog(LOG_CRIT, "smlun: Handling events in session.c(*handle_events).");
    ...
    while (s != NULL) {
        if (s->protocol == IPPROTO_ICMP || s->protocol == IPPROTO_ICMPV6) {
            if (!s->icmp.stop) {
                ...
                syslog(LOG_CRIT, "smlun: Creating packet in session.c using ICMP protocol.");
            ...
            if (s->udp.state == UDP_ACTIVE) {
                ...
                syslog(LOG_CRIT, "smlun: Creating packet in ip.c using UDP protocol.");
                jobject objPacket = create_packet(
            ...
            if (s->tcp.state != TCP_CLOSING && s->tcp.state != TCP_CLOSE) {
                ...
                syslog(LOG_CRIT, "smlun: Creating packet in session.c using TCP protocol.");
                jobject objPacket = create_packet(

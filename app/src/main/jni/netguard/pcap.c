...
void write_pcap_hdr() {
    syslog(LOG_CRIT, "[pcap.c, write_pcap_hdr()] Entering write_pcap_hdr");
...
void write_pcap_rec(const uint8_t *buffer, size_t length) {
    syslog(LOG_CRIT, "[pcap.c, write_pcap_rec()] Entering write_pcap_rec");
    struct timespec ts;
...
void write_pcap(const void *ptr, size_t len) {
    syslog(LOG_CRIT, "[pcap.c, write_pcap()] Entering write_pcap");
